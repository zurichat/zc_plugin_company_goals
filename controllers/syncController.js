...
exports.sync = async (req, res) => {
  try {
    ...
    for (let i = 0; i < userList.length; i += 1) {
      ...
      const findOrgs = await find('org_ids', { org_id: organization_id }, goalsOrgId);
      const { data } = findOrgs.data;

      if (userList[i].event === 'enter_organization') {
        if (!data || data === null || data.length < 1) {
          await insertOne('org_ids', { org_id: organization_id, member_ids: [member_id] }, goalsOrgId);
        } else {
          const newMemberIds = data[0].member_ids;

          if (!newMemberIds.includes(member_id)) newMemberIds.push(member_id);
          await updateOne('org_ids', { member_ids: newMemberIds }, { org_id: organization_id }, goalsOrgId);
        }
