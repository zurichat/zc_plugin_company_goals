...
const installPlugin = async (req, res) => {
  ...
  try {
    ...
    if (error.isOperational) return res.status(error.statusCode).send({ error: error.status });
    if (error.isAxiosError) return res.status(400).send({ message, success: false, data: null });
    res.status(500).json({
