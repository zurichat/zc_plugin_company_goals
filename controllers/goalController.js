...
exports.getReports = catchAsync(async (req, res, next) => {
  ...
  // Get specific goals
  const goals = await findAll('goals', orgId).data.data.length;
  const completedGoals = await find('goals', { isCompleted: true }, orgId).data.data.length;
  const unCompletedGoals = await find('goals', { isCompleted: false }, orgId).data.data.length;
  const expiredGoals = await find('goals', { expired: true }, orgId).data.data.length;
